...
""",
            countQuery = """
select count(distinct cast(function('date_format', coalesce(i.paidAt, i.dueDate), '%Y-%m') as string))
from Invoice i
where i.status = com.hyudequeue.genglish.tuition_fee_manager.entities.Enums.InvoiceStatusEnum.PAID
"""
    )
...
""")
    ...
    @Query(value = """
        SELECT 
            DATE_FORMAT(COALESCE(paid_at, due_date), '%Y-%m') as period,
            COALESCE(SUM(total_amount), 0) as revenue
        FROM invoices
        WHERE status = 'PAID'
          AND class_id = :classId
        GROUP BY DATE_FORMAT(COALESCE(paid_at, due_date), '%Y-%m')
        ORDER BY period DESC
        """,
            countQuery = """
        SELECT COUNT(DISTINCT DATE_FORMAT(COALESCE(paid_at, due_date), '%Y-%m'))
        FROM invoices
        WHERE status = 'PAID'
          AND class_id = :classId
        """,
    ...
    @Query(value = """
        SELECT 
            DATE_FORMAT(COALESCE(i.paid_at, i.due_date), '%Y-%m') as period,
        ...
        GROUP BY DATE_FORMAT(COALESCE(i.paid_at, i.due_date), '%Y-%m')
        ORDER BY period DESC
        """,
            countQuery = """
        SELECT COUNT(DISTINCT DATE_FORMAT(COALESCE(i.paid_at, i.due_date), '%Y-%m'))
        FROM invoices i
        INNER JOIN invoice_category_map icm ON i.invoice_id = icm.invoice_id
        WHERE i.status = 'PAID'
          AND icm.category_id = :categoryId
